...
public class JdbcClientRunRepository {
    ...
    public void update (Run run, Integer id) {
        var result = jdbcClient.sql("UPDATE Run SET title = ?, started_on = ?, completed_on = ?, miles = ?, location = ? WHERE id = ?")
                .params(List.of(run.title(), run.startedOn(), run.completedOn(), run.miles(), run.location(), id))
                .update();

        Assert.state(result == 1, "Run not updated" + run.title());
    }

    public void delete (Integer id) {
        var result = jdbcClient.sql("DELETE FROM Run WHERE id = :id")
                .param("id", id)
                .update();

        Assert.state(result == 1, "Run not deleted" + id);
    }
